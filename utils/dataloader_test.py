...
from data.physics.physics_data import PhysicsDataset, get_dataloader as get_physics_dataloader
from utils.dataloader import Dataloader, get_dataloader as get_custom_dataloader

def compare_dataloaders(physics_dataloader, custom_dataloader, num_batches=5):
    print("Comparing dataloaders...")
    
    for batch_idx, (physics_batch, custom_batch) in enumerate(zip(physics_dataloader, custom_dataloader)):
        ...
        print(f"\nBatch {batch_idx + 1}:")
        print(f"Physics input shape: {physics_input.shape}, Custom input shape: {custom_input.shape}")
        print(f"Physics target shape: {physics_target.shape}, Custom target shape: {custom_target.shape}")
...
def main():
    ...
    # Create dataloaders
    batch_size = 32
    physics_dataloader = get_physics_dataloader(input_file, target_file, batch_size, device='cpu', distributed=False)
    custom_dataloader = get_custom_dataloader(data, task='mujoco-v2', bsz=batch_size, distributed=False, preprocess=False, device='cpu')
