...
def main():
    ...
    d_model = (
        24  # TODO: Needs to be divisible by 8 otherwise custom conv1d lib won't work
    )
    d_out = 18
    d_state = 32
    d_conv = 4
    ...
    for epoch in range(num_epochs):
        for step, (xb, yb) in enumerate(train_loader):
            ...
            # Evaluate the loss
            preds, loss = model(
                inputs=xb, targets=yb, inference_params=inference_params
            )
            loss, _ = loss
            ...
            if (total_steps % eval_interval == 0) or total_steps == num_epochs * len(
                train_loader
            ) - 1:
                ...
                if val_loss < best_val_loss:
                    ...
                    patience_counter = 0
                    torch.save(
                        model.state_dict(), f"best_{controller}_mamba2.safetensors"
                    )
                else:
                    patience_counter += 1
