...
class DilatedCausalSelfAttention(MultiheadAttention):
    ...
    def forward(
        ...
        if incremental_state is not None:
            ...
            incremental_state["prev_key"] = k.view(bsz, self.n_heads, -1, self.head_dim)
        ...
        outs, lses = [], []
        for sl, dr in zip(
            self.configs.segment_lengths, self.configs.dilated_ratios, strict=True
        ):
            ki = self.gathering(
                k,
                dr,
                ...
                offset=0,
                is_kv=True,
                seq_parall=self.configs.seq_parallel,
            )
            vi = self.gathering(
                v,
                dr,
